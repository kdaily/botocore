...
def create_profile_string(endpoint_url=None, service_endpoints=None):
    ...
    if endpoint_url or service_endpoints:
        profile_str += "services = my-services\n\n"
        profile_str += "[services my-services]\n"
...
class TestOtherClientStuff:
    ...
    def test_env_service_and_global(self):
        """
        Set the S3-specific env variable and a global env endpoint.

        For a S3 client, the service specific env variable should 
        take precedence over the global env var.

        For a kms client, the global env var should be used.
    ...
    def test_s3_2(self, mock_botocore_session):
        ...
        with temporary_file('w') as f, \
            ...
            mock_botocore_session.set_config_variable('profile', 'test')
