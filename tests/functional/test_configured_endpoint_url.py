...
def create_cases(cases):
    ...
    for test in tests:
        ...
        service_section_str = dict_to_ini_section(
            service_section, section_name='services', name=service_section_name
        )
...
def test_resolve_custom_endpoint_url(
    test_case, client_config: bool, mock_botocore_session
):
    ...
    with temporary_file('w') as f, mock.patch.dict(
        ...
        if client_config:
            ...
            client = mock_botocore_session.create_client(test_case['service'])
